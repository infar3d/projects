...
def constructBayesNet(gameState: hunters.GameState):
    ...
    possible_obs = []
    for val in range(X_RANGE + Y_RANGE + 6):
        possible_obs.append(val)

    variableDomainsDict["Pacman"] = possible_pos
    variableDomainsDict["Ghost0"] = possible_pos
    variableDomainsDict["Ghost1"] = possible_pos
    variableDomainsDict["Observation0"] = possible_obs
    variableDomainsDict["Observation1"] = possible_obs
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            factors, new = joinFactorsByVariable(factors, var)
            if len(new.unconditionedVariables()) > 1:
                factors.append(eliminate(new, var))
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in positions:
            distance = self.getPositionDistribution(gameState, oldPos)
            for newPos in positions:
                distribution[newPos] += distance[newPos] * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        """     
        self.particles = []   
        distribution = self.numParticles // len(self.legalPositions)
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        new_beliefs_distribution = DiscreteDistribution()
        pac = gameState.getPacmanPosition()
        jail = self.getJailPosition()

        for particle_location in self.particles:
            new_beliefs_distribution[particle_location] += self.getObservationProb(observation, pac, particle_location, jail)

        if new_beliefs_distribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.beliefs = new_beliefs_distribution
            self.beliefs.normalize()
            self.particles = []
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.particles:
            new.append(self.getPositionDistribution(gameState, pos).sample())
        self.particles = new
