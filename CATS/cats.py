...
def about(topic):
    ...
    def function(param):
        modified=split(lower(remove_punctuation(param)))
        for x in topic:
            if x in modified:
                return True
        return False
...
def accuracy(typed, source):
    ...
    for x in range(0,min(len(typed_words),len(source_words))):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    word_dict={word: diff_function(typed_word,word,limit) for word in word_list}
...
def feline_fixes(typed, source, limit):
    ...
    def helper(i,letters_to_check):
        if letters_to_check>=0 and i<len(typed) and i<len(source):
            if typed[i]==source[i]:
                i+=1
                return helper(i,letters_to_check)
            else:
                i+=1
                letters_to_check=letters_to_check-1
                return helper(i,letters_to_check) + 1
        else:
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit < 0:  # Fill in the condition
        ...
        add = minimum_mewtations(start, goal[1:], limit-1) + 1  #we need to add one more letter to add
...
def report_progress(typed, prompt, user_id, upload):
    ...
    # END PROBLEM 8
    i=0
    range_of_func=range(0,len(typed))
    for elem in range_of_func:
...
def time_per_word(words, times_per_player):
    ...
    for x in times_per_player:
         lst.append([x[y] - x[y-1] for y in range(1,len(x))])
...
def fastest_words(match):
    ...
    lst=[[] for elem in player_indices]
    
    for x in word_indices:
        minimum=0
        for y in player_indices:
            if time(match,minimum,x) > time(match,y,x):
                minimum=y
        lst[minimum].append(get_word(match,x))
    return lst
