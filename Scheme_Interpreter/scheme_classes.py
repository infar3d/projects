...
class Frame:
    ...
    def __init__(self, parent):
        ...
        self.bindings = {} #dictionary representing the bindings
        self.parent = parent
    ...
    def make_child_frame(self, formals, vals):
        ...
        """
        if len(formals) != len(vals): #if the number of argument values doesn't match the number of formal parameters, raise the Scheme error
            raise SchemeError('Incorrect number of arguments to function call')
        # BEGIN PROBLEM 8
        new_frame = Frame(self)

        for formal in range(len(formals)):
