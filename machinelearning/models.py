...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        layer_one = nn.AddBias(nn.Linear(x,self.w1), self.b1)
        hidden = nn.ReLU(layer_one)
        layer_two = nn.AddBias(nn.Linear(hidden,self.w2),self.b2)
        return layer_two
    ...
    def train(self, dataset):
        ...
        while not accuracy:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                if loss > 0.02:
                    accuracy = False
                    losses = self.get_loss(nn.Constant(dataset.x),nn.Constant(dataset.y))
                    gradients = nn.gradients(losses, [self.w1, self.b1, self.w2, self.b2])
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.w1 = nn.Parameter(self.num_chars, self.d)
        self.w_hidden = nn.Parameter(self.d, self.d)
        self.w2 = nn.Parameter(self.d, 5)
    ...
    def run(self, xs):
        ...
        for letter in xs[1:]:
            layer_one = nn.ReLU(nn.Linear(letter, self.w1))
            layer_two = nn.ReLU(nn.Linear(z, self.w_hidden))
            z = nn.Add(layer_one, layer_two)
    ...
    def train(self, dataset):
        ...
        learning = True
        while learning:
            for x, y in dataset.iterate_once(100): #batch size is 50
                losses = self.get_loss(x, y)
                gradients = nn.gradients(losses, [self.w1, self.w_hidden, self.w2])
                self.w1.update(gradients[0], -self.learning_rate)
