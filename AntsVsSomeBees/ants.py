...
class Insect:
    ...
    def reduce_health(self, amount):
        ...
        has no health remaining. DAMAGE3
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None: #if there's no ant in the place 
            place.ant = self
        else:
            # BEGIN Problem 8b
            ...
            if self.is_container and self.can_contain(place.ant):  
                self.store_ant(place.ant) #then actually store that ant there 
                place.ant = self #then place that ant into that place
            ...
            if not curr.is_hive:
                if len(curr.bees) > 0:
                    if distance >=self.lower_bound:
                        bee_at_position = random_bee(curr.bees)
                        return bee_at_position 
            curr = curr.entrance
            distance +=1
        return None
...
class FireAnt(Ant):
    ...
    name = 'Fire'
    damage = 3 #the damage when an ant dies (the impact on the bee)
    food_cost = 5
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 5
    implemented = True  # Change to True to view in the GUI
    ...
    def reduce_health(self, amount):
        ...
        bees_lst = self.place.bees[:] #creating a copy of the bees that are in the row list
       

        for bee in bees_lst: #for every bee that exists on the row 
            if self.health <= amount: #if the FireAnt's health is not able to sustain the damage inflicted
                bee.reduce_health(self.damage + amount) #then we're also reducing the bee's health by the same amount (affects all bees in the row )
            else: #if the ant didn't die 
                bee.reduce_health(amount) #then we're reducing the bee's health by the amount that was inflicted by the bee on the ant 
...
class HungryAnt(Ant):
    ...
    food_cost = 4
    implemented = True
    chewing_turns = 3 #decrements with each turn 
    ...
    def action(self,gamestate): 
        if self.turns_to_chew == 0: #if we're done chewing the last bee and ready to eat another
            num_bees = len(self.place.bees)
            if num_bees>0: 
                bee = random_bee(self.place.bees)
                bee.reduce_health(bee.health)
                self.turns_to_chew = self.chewing_turns #then we are now chewing and need to start our chewing turns 
        else:
            self.turns_to_chew -= 1 #we're still chewing the previous bee and can't deal damage 
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.ant_contained is not None:
            ...
            #perform the ant_contained's action 
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if gamestate.instance: #this checks to see if we already have a queen ant 
            return None 
        else:
            gamestate.instance = True
    ...
    def action(self, gamestate):
        ...
        position = self.place.exit #checking if there is an ant behind you 
        while position != None:
            if position.ant != None: 
                position.ant.double()
                if position.ant.is_container:
                    if position.ant.ant_contained != None:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        regular_bee_action = target.action #Original Bee Action
        def slow(gamestate, counter = []): #How to iteratively keep track of counter 
            if len(counter) < 5: #Confirm duration (5) is accurate 
                if gamestate.time % 2 != 1: 
                    regular_bee_action(gamestate)
                counter.append("turn")
            else:
                regular_bee_action(gamestate) #When you do the call to Bee.action (target.action) what parameters are passed 
        target.action = slow #this will slow the bee, takeover of the orignal action 
